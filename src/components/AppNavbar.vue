<template>
  <nav
    ...
    <div
      ...
      <div :class="['order-2 flex items-center gap-1 sm:gap-2 md:gap-2.5 transition-all duration-200', searchExpanded ? 'max-[639px]:flex-1' : '']">
        ...
        <div
          ...
          <form @submit.prevent="submitSearch" role="search" :class="{ 'max-[639px]:hidden': !searchExpanded }" class="relative">
            ...
            <button
              ...
              @click="clearSearch"
              class="absolute end-8 sm:end-10 top-1/2 -translate-y-1/2 rounded-lg p-1 text-blue-500 transition hover:bg-blue-50 hover:text-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-blue-300 dark:hover:bg-blue-950/40 dark:hover:text-blue-200">
            ...
            <button
              ...
              class="absolute end-1 sm:end-2 top-1/2 -translate-y-1/2 rounded-lg p-1 text-gray-400 transition hover:bg-gray-100 hover:text-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:hover:bg-gray-700 dark:hover:text-blue-400">
              <svg class="h-4 w-4 sm:h-5 sm:w-5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="relative flex items-center gap-1 sm:gap-2">
          ...
          <button
            ...
            class="inline-flex items-center justify-center rounded-xl border border-gray-300 bg-white h-9 w-9 text-blue-500 transition hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-blue-300 dark:hover:bg-gray-700 sm:hidden">
            <svg class="h-4 w-4 sm:h-5 sm:w-5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35m2.1-5.4a7.5 7.5 0 1 1-15 0 7.5 7.5 0 0 1 15 0Z" />
            </svg>
          </button>
      ...
      <div class="relative" ref="localeDropdown">
        <button
          ...
          class="inline-flex h-9 items-center justify-between rounded-xl border border-gray-300 bg-white px-2.5 text-xs font-semibold text-gray-900 transition hover:bg-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700 sm:text-sm">
          <span class="sm:hidden">{{ currentLocaleShort }}</span>
          <span class="hidden sm:inline">{{ currentLocaleLabel }}</span>
          <svg
            class="ms-1 h-3.5 w-3.5 sm:h-4 sm:w-4"
            viewBox="0 0 20 20"
            fill="currentColor"
        ...
        <!-- Dropdown Menu (هم‌اندازه با دکمه بالایی) -->
        <div
          v-if="localeDropdownOpen"
          class="absolute start-0 end-0 z-20 mt-2 w-full min-w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
          <button
            type="button"
            @click="selectLocale('en')"
            :class="[
              'w-full text-center px-1.5 py-2 text-xs sm:text-sm font-medium transition',
              isLocale('en')
          ...
          <button
            ...
            class="inline-flex items-center justify-center h-9 w-9 text-sm text-gray-500 rounded-xl lg:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
